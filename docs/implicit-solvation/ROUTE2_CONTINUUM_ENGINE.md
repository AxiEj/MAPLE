...
| PySCF 2.13.1 SWIG/ISWIG IEFPCM | Published SWIG/ISWIG surface and analytic PCM nuclear gradients | `pyscf.solvent.pcm.gen_surface` removes points for which `weight * switch <= 1e-16`. The finite atom-centred Lebedev orientation also failed the existing second-molecule rigid-rotation qualification. |
