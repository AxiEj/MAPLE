...
| Frozen gas GFN2-xTB external-field screen | [`route2-v0-gfn2-xtb-acetone-field-prereg-v1.json`](benchmarks/route2-v0-gfn2-xtb-acetone-field-prereg-v1.json): **preregistered; not executed** | The installed official GFN2-xTB 6.7.1 exposes a self-consistent gas energy and external point-charge embedding, but its `--alpha` output is a D4 atomic-polarizability property and its local `--efield` implementation rejects GFN2. The preregistration therefore uses only a frozen, asymptotically uniform pair-charge finite-field response and the already frozen QM acetone reference. | This is the only no-runtime-QM electronic candidate currently under physical screening. It is not an xTB-COSMO/ALPB path, not a replacement of the public MACE route, and cannot enter a PCM or accuracy test unless it first passes the registered gas-response gates. |
