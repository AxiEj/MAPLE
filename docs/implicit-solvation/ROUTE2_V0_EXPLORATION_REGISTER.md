...
| V0-AQ-C open iso-density-product cavity composition | [`route2_v0_open_iso_density_cavity.py`](../../maple/function/calculator/extra_correction/implicit/route2_v0_open_iso_density_cavity.py) and [`test_route2_v0_open_iso_density_cavity.py`](../../tests/solvation/test_route2_v0_open_iso_density_cavity.py): **pass, structural only** | A centered, nonnegative, centrosymmetric relative kernel is applied by zero-extended linear convolution, so an isolated source cannot wrap through the opposite box face. The same declared map supplies the occupancy VJP; direct-loop convolution, weighted adjoint, cavity and composed-reaction finite differences, a deliberately non-neutral source, and AO-density dual pullback pass. | Retain only as the required open-domain calculus between a future stationary AO density and the open reaction scalar. Its test kernel/threshold are arbitrary structural controls, not a physical solvent asset; grid/buffer convergence, a stationary electronic functional, nonpolar scalar, force/PES certificate, speed result, and every chemistry/accuracy gate remain open. |
| V0-AQ-C open weighted-density cavitation scalar | [`route2_v0_open_weighted_density_cavity.py`](../../maple/function/calculator/extra_correction/implicit/route2_v0_open_weighted_density_cavity.py) and [`test_route2_v0_open_weighted_density_cavity.py`](../../tests/solvation/test_route2_v0_open_weighted_density_cavity.py): **pass, structural only** | The finite-box cavitation scalar applies an open normalized shell convolution to the occupancy deficit (s_{\rm c}-1), thereby continuing its exterior to bulk liquid instead of a periodic image or zero-occupancy vacuum. Direct-loop and adjoint identities, bulk/finite-void boundary behavior, central derivative, and crossed-state rejection pass. | Retain only as the open nonpolar calculus candidate. The test shell and pure-liquid numbers are not an admitted solvent asset, and no relation between (s_{\rm c}) and the electrostatic density-defined occupancy is assumed; physical shell/thermodynamic provenance, Pauli/dispersion, stationary electronics, force/PES, speed, and all accuracy gates remain open. |
