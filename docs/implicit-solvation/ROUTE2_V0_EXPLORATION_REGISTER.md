...
The currently screened **direct-drop-in** candidate frontier is closed for V0: official MACE-MDP is explicitly property-only (not an energy/force model), whereas MACE-Field and FieldSchNet-style common field-energy models require either an inorganic-domain checkpoint or dedicated response training. A separately designed MACE-MDP moment functional would be a new, preregistered research construction rather than a drop-in use of the checkpoint, and is not available in the pinned runtime. Those findings are recorded in the no-fit decision table rather than being silently treated as candidates.
