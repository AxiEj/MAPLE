...
| Official MACE-MDP response model | An official pretrained organic dipole/polarizability predictor; its documentation still declares it for those properties **only**, not energies or forces. The pinned generic `MACECalculator(model_type="DipolePolarizabilityMACE")` loads the frozen checkpoint, the sealed acetone coefficient screen passes raw-tensor symmetry/positivity plus the predeclared QM gates (relative Frobenius \(0.003868\), trace \(0.998759\), principal error \(0.004106\)), its atom-resolved polarizability contributions reconstruct the public tensor and produce \(\sum_a\alpha_a\alpha^{-1}=I\) to \(4.06\times10^{-17}\), and its full-Cartesian property derivatives match finite differences at \(2.28\times10^{-8}\) (dipole) and \(3.98\times10^{-8}\) (polarizability). | **Admit frozen \(\alpha_\theta(\mathbf R)\), its identity-bound atom moment partition, and explicit coefficient derivatives only; not a drop-in electronic functional** | It cannot directly serve as the same \(F(\mathbf R,c)\) that generates response and force. A moment-space quadratic built from the frozen \(\alpha\) is a *new* scalar, not a recovery of a common pretrained MACE energy. The atomic partition does not choose a GTO width or density source, and a property derivative is not a force. It may proceed only with a fixed energy-conjugate radial representer, positive Schur complement, common PCM scalar, all coefficient/source/cavity/gas-energy envelope terms, and full force/PES gates. The direct head remains excluded from production energy/force reporting and from all accuracy panels. [Coefficient artifact](benchmarks/route2-v0-mace-mdp-acetone-response-v1.json); [partition artifact](benchmarks/route2-v0-mace-mdp-atomic-map-acetone-v1.json); [derivative artifact](benchmarks/route2-v0-mace-mdp-acetone-derivatives-v1.json); [MACE docs](https://mace-docs.readthedocs.io/en/latest/guide/foundation_models.html) |
