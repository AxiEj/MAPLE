...
| Official MACE-MDP response model | An official pretrained organic dipole/polarizability predictor; its documentation still declares it for those properties **only**, not energies or forces. The pinned generic `MACECalculator(model_type="DipolePolarizabilityMACE")` loads the frozen checkpoint, the sealed acetone coefficient screen passes raw-tensor symmetry/positivity plus the predeclared QM gates (relative Frobenius \(0.003868\), trace \(0.998759\), principal error \(0.004106\)), and a separate frozen readout audit proves that its atom-resolved polarizability contributions reconstruct the public tensor and produce \(\sum_a\alpha_a\alpha^{-1}=I\) to \(4.06\times10^{-17}\). | **Admit frozen \(\alpha_\theta(\mathbf R)\) and its identity-bound atom moment partition only; not a drop-in electronic functional** | It cannot directly serve as the same \(F(\mathbf R,c)\) that generates response and force. A moment-space quadratic built from the frozen \(\alpha\) is a *new* scalar, not a recovery of a common pretrained MACE energy. The atomic partition does not choose a GTO width or density source. It may proceed only with a fixed energy-conjugate radial representer, positive Schur complement, common PCM scalar, differentiable coefficient/source/cavity derivatives, and force/PES gates. The direct head remains excluded from production energy/force reporting and from all accuracy panels. [Coefficient artifact](benchmarks/route2-v0-mace-mdp-acetone-response-v1.json); [partition artifact](benchmarks/route2-v0-mace-mdp-atomic-map-acetone-v1.json); [MACE docs](https://mace-docs.readthedocs.io/en/latest/guide/foundation_models.html) |
