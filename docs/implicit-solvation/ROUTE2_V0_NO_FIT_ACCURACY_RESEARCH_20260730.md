...
| Official MACE-MDP response model | An official pretrained organic dipole/polarizability predictor, but its own documentation declares it for those properties **only**, not energies or forces. | **Not a drop-in V0 electronic functional; deferred construction** | It cannot directly serve as the same \(F(\mathbf R,c)\) that generates the response and force. A moment-space quadratic built from predicted \(\mu,\alpha\) would be a *new*, separately preregistered functional with its own positive-definiteness, density/PCM pairing, and force gates; it is not a recovery of a common pretrained energy. The pinned `mace-torch==0.3.16` runtime also has no `mace_mdp` loader. [MACE docs](https://mace-docs.readthedocs.io/en/latest/guide_models.html) |
