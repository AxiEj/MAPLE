...
| Official MACE-MDP response model | An official pretrained organic dipole/polarizability predictor; its documentation still declares it for those properties **only**, not energies or forces. The pinned generic `MACECalculator(model_type="DipolePolarizabilityMACE")` loads the frozen checkpoint, and the sealed acetone coefficient screen passes raw-tensor symmetry/positivity plus the predeclared QM gates (relative Frobenius \(0.003868\), trace \(0.998759\), principal error \(0.004106\)). | **Admit frozen \(\alpha_\theta(\mathbf R)\) coefficients only; not a drop-in electronic functional** | It cannot directly serve as the same \(F(\mathbf R,c)\) that generates response and force. A moment-space quadratic built from the frozen \(\alpha\) is a *new* scalar, not a recovery of a common pretrained MACE energy. It may proceed only with a fixed energy-conjugate induced-density representer, positive Schur complement, common PCM scalar, differentiable coefficient/source/cavity derivatives, and force/PES gates. The direct head remains excluded from production energy/force reporting and from all accuracy panels. [MACE-MDP artifact](benchmarks/route2-v0-mace-mdp-acetone-response-v1.json); [MACE docs](https://mace-docs.readthedocs.io/en/latest/guide/foundation_models.html) |
