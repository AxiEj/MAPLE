...
| V0-RK response-kernel completion (proposed fallback) | Start from an independently source-bound positive semidefinite GTO density-response kernel \(C_0\), an atom-dipole map \(A\), and the frozen MACE partition \(W\). On the physical support, replace only the atom-dipole covariance \(S_0=AC_0A^\mathsf T\) by \(\Gamma=W\alpha_\theta W^\mathsf T\): \(C=C_0-C_0A^\mathsf TS_0^{-1}AC_0+C_0A^\mathsf TS_0^{-1}\Gamma S_0^{-1}AC_0\). Then \(C\succeq0\), \(ACA^\mathsf T=\Gamma\), and the global dipole response is exactly \(\alpha_\theta\). The quadratic electronic scalar on the response support is \(\tfrac12\delta c^\mathsf TC^+\delta c\). | **Fallback no-fit source research gate only; not implemented, not admitted to PCM, and not an accuracy result** | This retains a physically sourced radial/null-space response while using the frozen MACE tensor only for its observed molecular moment response. It is a defined low-rank covariance replacement, not a width fit or response correction. The baseline \(C_0\) must be fixed from independent electronic-response data before seeing any solvation label, and the completed kernel must first pass preregistered QM MEP, density, reciprocity, passivity, force, and speed gates. |
