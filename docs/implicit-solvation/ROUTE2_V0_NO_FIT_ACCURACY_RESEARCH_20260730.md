...
| Frozen gas GFN2-xTB external pair-field response | The installed point-charge finite-field interface does not make its reported full dipole the energy-conjugate source. | **Rejected as a V0 response/electronic-functional candidate before PCM or accuracy** | The preregistered no-runtime-QM acetone screen is fast (13 cases: 0.242 s versus the archived QM finite-field reference: 855.736 s) but fails both required layers: its dipole polarizability has \(0.45046\) relative Frobenius mismatch and \(0.54691\) trace ratio against QM, and its energy/dipole diagonal identity misses by \(0.48611\). This is not repairable by rescaling, parameter adjustment, or an external correction. xTB COSMO/GBSA/ALPB/CPCM-X remains excluded. A distinct, zero-field MOLDEN permanent-source audit may use its stationary valence density **only as an affine origin**, never this rejected response; see [`ROUTE2_V0_GFN2_MOLDEN_PERMANENT_SOURCE_THEORY.md`](ROUTE2_V0_GFN2_MOLDEN_PERMANENT_SOURCE_THEORY.md). |
