#!/usr/bin/env python3
"""Generate the reproducible v2 free-atom translation-tangent response asset.
...
# Set the numerical thread environment before NumPy or PySCF can load a BLAS/OpenMP
# runtime.  v1 did not freeze this and produced non-identical free-atom tables.
# v2 is intentionally a one-thread source-falsification protocol, not a
# performance path.
import os
...
V0_ATOMIC_DISPLACEMENT_RESPONSE_ARTIFACT = (
    "route2-v0-atomic-displacement-hf-def2-tzvpd-v2"
)
...
RUNNER_RELATIVE_PATH = (
    "docs/implicit-solvation/benchmarks/"
    "generate_route2_v0_atomic_displacement_response_v2.py"
)
...
def _deterministic_npz_bytes(arrays: dict[str, np.ndarray]) -> bytes:
    """Serialize arrays without wall-clock ZIP metadata.

    The v1 table used ``numpy.savez_compressed``, whose ZIP metadata was not
    part of a reproducibility contract.  The v2 source is deliberately small,
    so make both the numerical runtime and the archive representation explicit.
    """
...
def _write_exclusive_deterministic_npz(
    path: Path,
...
def _validate_preregistration(
    ...
    *,
    ...
    if not isinstance(runtime, dict):
        raise TypeError("The atomic-displacement preregistration lacks a runtime.")
    ...
    if (
        ...
        raise RuntimeError(
            "The atomic-displacement numerical runtime changed after freeze."
        )
    if contract.get("deterministic_archive") != DETERMINISTIC_ARCHIVE_CONTRACT:
        raise RuntimeError(
            "The atomic-displacement deterministic archive contract changed."
        )
...
def main() -> int:
    ...
    protocol, runtime_contract = _validate_preregistration(
    ...
    active_pyscf_threads = int(lib.num_threads())
    if active_pyscf_threads != arguments.threads:
        raise RuntimeError(
            "PySCF did not honor the preregistered atomic-displacement thread count."
        )
    ...
    _write_exclusive_deterministic_npz(arguments.output, arrays)
    manifest = {
        ...
        "runtime": {
            ...
            "pyscf_lib_num_threads": active_pyscf_threads,
            "python": sys.version,
    ...
    if manifest["runtime"] != {
        ...
        "python": sys.version,
        "python_resolved_sha256": _sha256(Path(sys.executable).resolve()),
        "thread_environment": runtime_contract["thread_environment"],
        "threads": runtime_contract["threads"],
    }:
        raise RuntimeError("The atomic-displacement runtime evidence is inconsistent.")
