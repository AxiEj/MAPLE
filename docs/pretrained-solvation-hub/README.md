...
### FeNNix-Bio1: real CPU/GPU mechanics, paper FE integration blocked
...
The unchanged FeNNol `2026.6.29` runtime and hash-pinned medium checkpoint have
now also been executed in an isolated environment.  The frozen runtime record
is
`benchmarks/fennix-bio1-runtime-smoke-2026-07-31.json`, regenerated by
`run_fennix_bio1_runtime_smoke.py`.  CPU and GPU are selected in separate
processes because JAX backend selection is process-global.  On the official
three-atom README water geometry, both paths requested x64 coordinates,
`highest` matrix precision, and float64 outputs without enabling a reduced
mode.  The serialized checkpoint parameters remain float32.  The paired
outputs are not bit-exact: the observed absolute energy difference is
`3.3306690738754696e-16 eV` and the maximum force-component difference is
...
python docs/pretrained-solvation-hub/run_fennix_bio1_runtime_smoke.py run \
  --python /absolute/path/to/the/pinned/fennol/python \
  --checkpoint /absolute/path/to/fennix-bio1M.fnx \
  --output-dir /absolute/path/to/runtime-receipts

python docs/pretrained-solvation-hub/run_fennix_bio1_runtime_smoke.py combine \
  --cpu /absolute/path/to/cpu-float64.json \
  --gpu /absolute/path/to/gpu-float64.json \
  --comparison /absolute/path/to/comparison.json \
  --alchemical-cpu /absolute/path/to/alchemical-kernel-cpu-float64.json \
  --alchemical-gpu /absolute/path/to/alchemical-kernel-gpu-float64.json \
  --alchemical-upcast-cpu /absolute/path/to/alchemical-kernel-cpu-upcast64.json \
  --alchemical-upcast-gpu /absolute/path/to/alchemical-kernel-gpu-upcast64.json \
  --alchemical-upcast-comparison /absolute/path/to/alchemical-kernel-upcast64-comparison.json \
  --maple-adapter /absolute/path/to/maple-adapter-cpu-float64.json \
  --pip-freeze /absolute/path/to/pip-freeze.txt \
  --nvidia-smi /absolute/path/to/nvidia-smi.csv \
  --artifact-sha256 /absolute/path/to/artifact-sha256.txt \
  --output /absolute/path/to/fennix-bio1-runtime-smoke.json
```
...
bridge-level precision switch.  The real mechanics receipts do not change the
scientific gate: no HFE sampling or Lambda-ABF estimate was run, no experimental
accuracy was evaluated, and no panel of at least ten records spanning at least
ten distinct primary functional groups was run.  Consequently GPU remains
fail-closed, no no-degradation claim is allowed, and matched-QM timing is
ineligible.  The artifact deliberately contains no speedup or performance
claim.  Exact end-to-end free-energy protocol reconstruction, experimental
validation, and task-by-task validation remain open.

A second mechanics-only diagnostic derived an in-memory float64 copy of every
floating checkpoint parameter without modifying the original checkpoint file.
Its five-state CPU/GPU scalar and gradient-summary differences are at most
`1.3322676295501878e-15`, but upcasting changes the original-model summaries by
as much as `3.537531757802359e-08`.  The receipt did not retain full force or
cell-gradient arrays.  It therefore defines a distinct derived runtime
identity and is not evidence of improved precision, experimental accuracy, or
GPU admission.
