...
| --- | :---: | :---: | :---: | :---: | :---: | --- |
| `route2-profile-diagnostic-aimnet2-geometry-mediated-ddx-ddpcm-electrostatic-v1` | no | no | no | no | no | explicit `R -> q_NQE(R) -> ddPCM` fixed-stratum scalar; deterministic metric reciprocity, apply/adjoint, charge-direction FD, gauge, hard-neighbor, and cavity-active-set audits are implemented. The real water audit is negative: its small three-step stencil stays in one stratum but lacks an admissible FD convergence window, and all three frozen rotations change the laboratory-grid cavity active set. Checkpoint-release provenance, measured pyddx post-solve residual, nonpolar physics, domain/loop/HVP/NVE, and public integration remain absent |
| `route2-profile-diagnostic-aimnet2-geometry-mediated-smoothharmonicgalerkin-cpcm-electrostatic-v1` | no | no | no | no | no | same unmodified AIMNet2 point charges coupled to a fixed-dimensional smooth-harmonic conductor scalar through the exact point-charge addition theorem. NumPy/Torch parity, same-scalar derivatives, reciprocity, stationarity residual, and structural symmetry tests pass; the real water rigid-rotation gate also passes without a laboratory grid. The real three-step full-energy directional gate remains negative, and this conductor reference has no finite-dielectric solvent identity, nonpolar term, domain/HVP/NVE evidence, or public admission |
