...
| --- | :---: | :---: | :---: | :---: | :---: | --- |
| `route2-profile-diagnostic-aimnet2-geometry-mediated-ddx-ddpcm-electrostatic-v1` | no | no | no | no | no | explicit `R -> q_NQE(R) -> ddPCM` fixed-stratum scalar; deterministic metric reciprocity, apply/adjoint, charge-direction FD, gauge, hard-neighbor, full-`3N` Cartesian, and cavity-active-set audits are implemented. A source-bound float64 real-water canary now records and replays every adaptive SciPy abscissa. Its derivative converges and agrees with the analytic gradient, but the independently reconstructed ddX active-set clearance is only `5.826e-9 A`, the adaptive stencil changes cavity stratum, and all three frozen rotations change the laboratory-grid active set. The legacy float32 derivative arm is independently negative. Checkpoint-release provenance, measured pyddx post-solve residual, nonpolar physics, domain/loop/HVP/NVE, and public integration remain absent |
| `route2-profile-diagnostic-aimnet2-geometry-mediated-smoothharmonicgalerkin-cpcm-electrostatic-v1` | no | no | no | no | no | same field-independent AIMNet2 point charges coupled to a fixed-dimensional smooth-harmonic conductor scalar through the exact point-charge addition theorem. NumPy/Torch parity, same-scalar derivatives, reciprocity, stationarity residual, point/source-shell plus sphere-tangency topology margins, and structural symmetry tests pass. The legacy float32 checkpoint graph remains a derivative negative control; an optional official-source-bound `aimnet==0.2.0` reconstruction loads unchanged weights in a float64 graph and passes one-water directional/full-Cartesian/rotation prerequisites. The exact v2 17-shard H/C/N/O distorted-geometry panel reuses the frozen MAPLE PES asset and forbids partial aggregation. Two complete clean-process sets replay every scientific measurement SHA exactly: only water and hydrogen peroxide pass every gate; four molecules fail the point/source-shell guard, ethylamine fails the separate sphere-tangency guard, and fourteen molecules lack a demonstrated refinement/plateau window over the frozen small-step scan. Three S/Cl controls remain explicitly outside the local checkpoint contract. A water bidirectional loop/event audit, a complete weak-scalar HVP canary, and a guarded stationary-water dense-Hessian/normal-mode canary each pass two clean processes with identical scientific measurements. The latest record closes nine HVP columns against a four-step total-gradient FD scan, Hessian symmetry, three translations, three stationary rotations, and the correctly mass-weighted three-mode subspace. This is still not task admission: the current-profile domain panel is negative, while broad event-free `C2`, multi-stationary-point/FREQ/TS/IRC evidence, finite-dielectric solvent identity, nonpolar physics, NVE, and public integration remain absent |
| `route2-profile-diagnostic-aimnet2-geometry-mediated-smoothharmonicgalerkin-ddpcm-electrostatic-v1` | no | no | no | no | no | disabled finite-dielectric sibling of the preceding harmonic conductor. It reuses the fixed-dimensional exposure, single-layer, point-source, SO(3), and same-scalar derivative machinery, and adds the ddPCM `R_epsilon Phi_epsilon=R_infinity Phi` double-layer stage rather than a uniform COSMO scale. The generally nonsymmetric primal apparent-charge response is retained as evidence but is not exposed as the provider field; the provider uses the transpose/KKT energy cotangent and audits its registered-metric self-adjointness, charge-tangent reciprocity, `sym(P_primal)` identity, half coupling, and all primal/adjoint residuals. Unit-level dielectric, derivative, HVP, symmetry, and fail-closed tests pass. A retained two-process real-checkpoint water canary now also passes adaptive and full-Cartesian gradient, rigid-rotation covariance, stationarity/residual, reciprocity, charge-gauge, and event-clearance gates with identical scientific SHA256. This closes a local finite-dielectric force diagnostic, but the identity remains a parameterized diagnostic: a solvent-bound profile, broad topology/refinement/PES panel, compatible nonpolar term, chemical-accuracy evidence, and public integration remain absent. All E/F/H/V/M and OPT/FREQ/TS/IRC/MD gates therefore remain closed |
| `route2-profile-candidate-aimnet2-frozen-charge-water-smoothharmonicgalerkin-ddpcm-electrostatic-v1` | no | no | no | no | no | exact water-bound successor to the parameterized diagnostic. It locks model profile `aimnet2-polarizable-v1` to unchanged checkpoint weights, one NQE charge evaluation per geometry, no continuum-field input and no electronic SCF; it also locks water `epsilon=78.355`, the SMD Coulomb-radius policy, transition width `0.18 A^2`, surface/exposure orders `1/2`, and quadrature order `32`. The latest source-bound float64 runtime retains the ordinary forward as a parity oracle and reapplies the same upstream DFT-D3 term through its smooth Hessian-capable graph. Two complete 17-molecule H/C/N/O processes reproduce every measurement: all frozen convergence checks pass and 12 molecules pass every gate, but methanol, methane, dimethyl ether, and acetic acid fail point/source-shell clearance and ethylamine fails sphere tangency. A separate ten-record MNSol pilot gives early aggregate accuracy for fixed pyddx electrostatics plus SMD-CDS, not for this `G_np=0` scalar. Water-only bidirectional-loop, complete-HVP, and guarded stationary dense-Hessian/frequency diagnostics also pass two clean processes each. These are substantial local force/task diagnostics, not public admission: the full domain panel remains topology-negative, compatible nonpolar physics, broad event-free `C2`, multi-stationary-point/FREQ/TS/IRC, NVE, and public integration remain absent |
| `route2-profile-candidate-aimnet2-frozen-charge-multisolvent-smoothpartitionharmonic-ddpcm-pyscf-smdcds-v1` | no | no | no | no | no | exact one-shot multi-solvent composite: gas-phase AIMNet2 `output['charges']` after NQE and only floating-residue cleanup, point-l0 smooth-partition ddPCM, and a separately evaluated PySCF 2.13.1 SMD-CDS component; no field is supplied to AIMNet2 and no electronic SCF occurs. The complete 653-record MNSol panel gives MAE/RMSE/bias `2.5817/3.3526/+2.5426 kcal/mol`, water MAE `3.4332`, and smooth-vs-high-resolution ddPCM MAE `0.0619`. Thus the smooth discretization is not the main accuracy deficit; the gas-phase Hirshfeld-like monopole representation plus hybrid ledger is a fixed-charge baseline, not original SMD or a literature-backed approximately 1 kcal/mol method. The worst parity record is also not harmonic-order converged at the frozen `4/8` orders. Accuracy, broad same-scalar force/path evidence, HVP, stress/virial semantics, task integration, and all admission gates remain closed |
...
Acetamide extends the contiguous range to `[0,13)`. Its two clean runs are
retained under
...
Its maximum rotation-energy and relative force-covariance errors are
`3.928107616957277e-9 eV` and `4.6556222143172366e-8`. Thirteen of twenty
equilibrium molecules are complete; seven remain and no capability changes.

Ethylamine extends the contiguous range to `[0,14)`. Its two clean runs are
retained under
...
Its maximum rotation-energy and relative force-covariance errors are
`3.653894964372739e-9 eV` and `4.700291638874309e-8`. Fourteen of twenty
equilibrium molecules are complete; six remain and no capability changes.

Pyridine extends the contiguous range to `[0,15)`. Its two clean runs are
retained under
...
Its maximum rotation-energy and relative force-covariance errors are
`4.036155587527901e-8 eV` and `6.934036634982884e-8`. Fifteen of twenty
equilibrium molecules are complete; five remain and no capability changes.

Nitromethane extends the contiguous range to `[0,16)`. Its two clean runs are
retained under
...
Its maximum rotation-energy and relative force-covariance errors are
`1.0765688784886152e-8 eV` and `1.5400011762629733e-8`. Sixteen of twenty
equilibrium molecules are complete; four remain and no capability changes.

Hydrogen peroxide extends the contiguous range to `[0,17)`. Its two clean runs
are retained under
...
Methanethiol extends the contiguous range to `[0,19)`. Its two clean runs are
retained under
...
Its maximum rotation-energy and relative force-covariance errors are
`6.270056474022567e-9 eV` and `2.3666944175900336e-8`. Nineteen of twenty
equilibrium molecules are complete; one remains and no capability changes.

Chloroform completes the contiguous range `[0,20)`. Its two clean runs are
retained under
...
Its maximum rotation-energy and relative force-covariance errors are
`3.2669049687683582e-9 eV` and `2.1676643473015314e-8`. Across the complete
twenty-molecule equilibrium panel, the respective maxima are
`4.2611645767465234e-8 eV` (benzene) and `6.934036634982884e-8` (pyridine).
This closes the frozen equilibrium rigid panel only; all distorted-geometry,
Cartesian-FD, loop-work, accuracy, Hessian, workflow, and release gates remain
open, so no capability changes.
