...
| `route2-profile-candidate-aimnet2-frozen-charge-water-smoothharmonicgalerkin-ddpcm-electrostatic-v1` | no | no | no | no | no | exact water-bound successor to the parameterized diagnostic. It locks model profile `aimnet2-polarizable-v1` to the unchanged source-bound float64 checkpoint graph, one NQE charge evaluation per geometry, no continuum-field input and no electronic SCF; it also locks water `epsilon=78.355`, the `smd-ddpcm-l15-n1202-multisolv-v1` Coulomb-radius policy, transition width `0.18 A^2`, surface/exposure orders `1/2`, and quadrature order `32`. Two clean processes under this exact identity reproduce scientific SHA256 `29ad72cd...08e5` and pass the same local water derivative, stationarity, symmetry, and event gates. The distinct identity and local canary remove configuration ambiguity but admit no capability. Broad distorted-PES/event/refinement evidence, compatible nonpolar physics, accuracy panels, and task-level integration are still required |
