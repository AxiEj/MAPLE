...
| `route2-profile-diagnostic-aimnet2-geometry-mediated-smoothharmonicgalerkin-ddpcm-electrostatic-v1` | no | no | no | no | no | disabled finite-dielectric sibling of the preceding harmonic conductor. It reuses the fixed-dimensional exposure, single-layer, point-source, SO(3), and same-scalar derivative machinery, and adds the ddPCM `R_epsilon Phi_epsilon=R_infinity Phi` double-layer stage rather than a uniform COSMO scale. The generally nonsymmetric primal apparent-charge response is retained as evidence but is not exposed as the provider field; the provider uses the transpose/KKT energy cotangent and audits its registered-metric self-adjointness, charge-tangent reciprocity, `sym(P_primal)` identity, half coupling, and all primal/adjoint residuals. Unit-level single-sphere dielectric limits, source/coordinate FD, joint HVP, rigid symmetry, and tangency fail-closed tests pass, but no retained clean-tree real-checkpoint artifact, solvent-bound admission identity, broad topology/refinement panel, nonpolar term, or public integration exists. All E/F/H/V/M and OPT/FREQ/TS/IRC/MD gates therefore remain closed |
