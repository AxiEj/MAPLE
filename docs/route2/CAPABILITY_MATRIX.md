...
| `route2-profile-diagnostic-aimnet2-geometry-mediated-smoothharmonicgalerkin-cpcm-electrostatic-v1` | no | no | no | no | no | same field-independent AIMNet2 point charges coupled to a fixed-dimensional smooth-harmonic conductor scalar through the exact point-charge addition theorem. NumPy/Torch parity, same-scalar derivatives, reciprocity, stationarity residual, point/source-shell plus sphere-tangency topology margins, and structural symmetry tests pass. The legacy float32 checkpoint graph remains a derivative negative control; an optional official-source-bound `aimnet==0.2.0` reconstruction loads unchanged weights in a float64 graph and passes one-water directional/full-Cartesian/rotation prerequisites. An exact v2 17-shard H/C/N/O distorted-geometry harness reuses the frozen MAPLE PES asset and forbids partial aggregation: water passes two clean v2 processes, but methanol reproducibly fails the bond-stretched point/source-shell event guard despite passing every numerical derivative gate; fifteen shards and three explicitly out-of-contract S/Cl controls remain unclaimed. A water bidirectional loop/event audit, a complete weak-scalar HVP canary, and a guarded stationary-water dense-Hessian/normal-mode canary each pass two clean processes with identical scientific measurements. The latest record closes nine HVP columns against a four-step total-gradient FD scan, Hessian symmetry, three translations, three stationary rotations, and the correctly mass-weighted three-mode subspace. This is still not task admission: the current-profile domain panel is already negative, while broad event-free `C2`, multi-stationary-point/FREQ/TS/IRC evidence, finite-dielectric solvent identity, nonpolar physics, NVE, and public integration remain absent |
