...
| `route2-profile-experimental-macemdppoint-macepolarinduced-smoothharmonicgalerkin-local-electrostatic-ef-v2` | yes | yes | no | no | no | exact local electrostatic H1 identity at clean commit `7561edfa`: two independent 141-event CUDA replays have identical measurement SHA-256 `4e60a259...9c38a9`, all frozen local gates pass, and standalone data-only overlay `32b05dea...eba863` admits only energy plus runtime-error-bounded numerical scalar-gradient force; static registry/safe defaults remain closed and no complete-solvation, chemical-accuracy, analytic-force, H/FREQ, MD, or Tier-V claim transfers |
...
| `route2-operational-macemdppoint-macepolarinduced-gto1p5-nativefield8-smoothharmonic-ddpcm-smd-total-v1` | no | no | no | no | no | distinct total scalar `Phi0_hyb + G_SMD-CDS`; its one allowed MNSol-10 known-panel attempt is permanently closed by aggregate-only failure `2b42f99d...57eb` before the first record was admitted: a fixed `1e-12 eV` subtraction closure rejected binary64 cancellation in `fl(Phi-V)` versus directly constructed `fl(H+CDS)`. No labels or accuracy metric were read, so this is numerical-contract negative evidence, not an accuracy result; no capability transfers or rerun are allowed. Pro-approved prospective exact-dyadic/ULP contract audit `d58f67d9...d6d7c1a` is infrastructure for a new 148-row preregistration only |
