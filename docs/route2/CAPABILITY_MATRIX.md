...
| `route2-profile-experimental-pure-macepolar-frozen-point-l1-ddpcm-smd-v1` | no | no | no | no | no | **experimental execution enabled; workflow/release tiers unadmitted** official CPU/float64 pure MACE-POLAR point-`l<=1` + pyddx ddPCM `l15/n1202/tol1e-12/eta0.1/nproc1` + PySCF 2.13.1 SMD-CDS scalar. The machine-readable execution surface exposes E, analytic-chain-rule F, molecular virial, and Richardson HVP/H; periodic stress is false, and registry `V` means variational rather than virial. The preregistered 505-row development energy panel has MAE `1.2850369252 kcal/mol` (hard target `<=1.5`, pass), but confirmation is sealed and energy error does not establish physical F/virial/H accuracy. PySCF CDS topology is only partially observable, so H/HVP require explicit experimental policy and remain workflow-unadmitted. Model impersonation is rejected; discretization/runtime overrides fall back to a generic unregistered scalar ID. |
| `maple.route2.experimental.mace-polar-frozen-source-ddx-smd.impl.v1` radial-GTO/194 canary | no | no | no | no | no | distinct low-cost water derivative canary using radial-GTO ddPCM/194 and smooth CDS; it does not inherit the point-profile 505-row accuracy result |
...
The first narrow experimental E/F target is now admitted only for the exact
MACE-MDP/MACE-POLAR harmonic profile above. Production electrostatic release
still requires physical-component and accuracy evidence plus an integrated
public workflow. Tier V is not required for operational E/F and must remain
false unless the model energy/source identity, reciprocity, stability,
invertibility, and full coordinate derivative are independently proven.
