...
| --- | :---: | :---: | :---: | :---: | :---: | --- |
| `route2-profile-diagnostic-aimnet2-geometry-mediated-ddx-ddpcm-electrostatic-v1` | no | no | no | no | no | explicit `R -> q_NQE(R) -> ddPCM` fixed-stratum scalar; deterministic metric reciprocity, apply/adjoint, charge-direction FD, gauge, hard-neighbor, full-`3N` Cartesian, and cavity-active-set audits are implemented. The legacy real-water audit is negative: its float32 energy differences lack an admissible convergence window, and all three frozen rotations change the laboratory-grid cavity active set. Checkpoint-release provenance, measured pyddx post-solve residual, nonpolar physics, domain/loop/HVP/NVE, and public integration remain absent |
