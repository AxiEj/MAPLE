...
| `route2-operational-macemdppoint-macepolarinduced-gto1p5-nativefield8-smoothharmonic-ddpcm-smd-total-v1` | no | no | no | no | no | distinct total scalar `Phi0_hyb + G_SMD-CDS`; the additive ledger preserves the exact finite-ddPCM source/root identity and subtracts the same vacuum term when scoring `Delta G_solv`. MNSol-10 is preregistered only as a known-panel regression; final accuracy admission requires the complete frozen 148-row confirmation partition and does not inherit the pure-route pass |
