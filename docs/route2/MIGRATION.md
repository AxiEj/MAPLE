...
| 6: force/MAPLE integration | one experimental electrostatic E/F profile admitted; stable production integration still blocked | the MACE-MDP permanent + MACE-POLAR induced + smooth harmonic profile passes replicated same-profile `h/h2/h4`, directional, translation, rotation-force, and guarded loop gates; the force is a runtime-guarded numerical scalar gradient, while chemical accuracy, complete solvation, analytic force, workflows, H/V/M remain closed |
| 7: Hessian/FREQ/TS | pending | no vNext Tier H profile |
| 8: CDS/multisolvent/performance | blocked by component/accuracy/workflow gates | the experimental conductor-limit E/F profile has no finite-dielectric solvent transfer or compatible CDS/nonpolar scalar |
