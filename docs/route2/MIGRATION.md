...
| 5: MACE-POLAR adapter/canaries | candidates complete; admission gate open | the original 194-node profile failed rotation/torque; fixed-box40/CPCM590 passed earlier derivative/path panels but failed the later frozen water rotation canary; a separate fixed-box48/CPCM1202 candidate is registered but unexecuted and unadmitted |
| 6: force/MAPLE integration | blocked | same-scalar force is callable internally and the preregistered directional, component-resolved, and residual-refinement panels pass, but the current CPCM590 profile fails the all-panel rotation gate; public calculator/workflow and admission gates remain closed |
