...
| 6: force/MAPLE integration | explicit experimental daily surface wired; production accuracy still blocked | `macemdppolarhybridddx` exposes named-solvent E/F, nonperiodic molecular virial, MAPLE SP, and first-order LBFGS/SD/SDCG/CG OPT without pretending that the accuracy target or strict Tier V passed |
| 7: Hessian/FREQ/TS | direct experimental H callable; FREQ/TS remain closed | H is an error-estimated Richardson derivative of the same analytic force under an explicit observed-components-only SMD-topology policy; the existing gas-phase thermochemistry driver is not exposed as solution-phase FREQ |
| 8: CDS/multisolvent/performance | named-solvent execution open; quantitative admission blocked | the daily calculator uses the existing Route-2 solvent registry and prevents implicit/D4 double counting; current 505-development accuracy remains `1.696313 kcal/mol` MAE and `14.904481 kcal/mol` maximum error |
