...
| 5: solute-model adapters/canaries | MACE-POLAR candidates complete with the original common-scalar, current molecular-realspace SO(3), and analytic changed-inference Tier-V stability routes closed negative; AIMNet2 geometry-mediated candidates implemented and unadmitted | the MACE evidence boundaries are unchanged; one AIMNet2 adapter binds geometry-dependent NQE monopoles and their coordinate VJP without pretending the model accepts a reaction field. It composes the reused pyddx atomic-l1 map, the smooth-harmonic conductor, or a distinct parameterized finite-dielectric harmonic ddPCM diagnostic. The latter reuses the same exposure/single-layer/point-source machinery, adds the published double-layer equation, and exposes only its transpose/KKT energy cotangent rather than the generally nonsymmetric primal apparent-charge response. The legacy float32 runtime remains a derivative negative control; the source-bound official-Python float64 reconstruction passes the retained conductor-reference prerequisites, while finite-ddPCM still lacks a retained real-stack artifact and every public task gate remains closed |
...
| 7: Hessian/FREQ/TS | local stationary-water implementation evidence complete; admission pending | the complete weak-scalar HVP is assembled from the sealed continuum joint Hessian and AIMNet2 intrinsic/charge response. One source-bound water HVP canary passes local FD/symmetry/translations, and a second guarded stationary-water canary passes nine-column gradient FD, Hessian symmetry, three stationary rotations, and a correctly mass-weighted three-mode subspace. No vNext Tier H profile, broad FREQ, TS, or IRC admission evidence exists |
