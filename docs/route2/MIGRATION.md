...
| 5: MACE-POLAR adapter/canaries | candidates complete; full gate open | the original 194-node profile retains its rotation/torque failure; a distinct fixed-box40/590-node diagnostic passes one-water Cartesian/orientation and distorted-water path/loop gates but lacks box convergence, the multi-molecule PES panel, and physical-component admission |
