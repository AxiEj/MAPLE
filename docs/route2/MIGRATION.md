...
| 1: contracts and units | engineering complete; current registry fully fail-closed | immutable API registries; ASE public eV/eV/A boundary; full regression suite; the historical `4cf8db40` E/F record is retained but not transferred across provider-byte drift |
...
| 6: force/MAPLE integration | historical narrow E/F record exists; current admission closed | the exact `4cf8db40` MACE-MDP permanent + MACE-POLAR induced + smooth harmonic profile passed replicated `h/h2/h4`, directional, translation, rotation-force, and guarded loop gates; current provider bytes differ, so E/F must be replayed twice before reopening, while chemical accuracy, complete solvation, analytic force, workflows, H/V/M remain closed |
| 7: Hessian/FREQ/TS | pending | no vNext Tier H profile |
| 8: CDS/multisolvent/performance | blocked by component/accuracy/workflow gates | the historical conductor-limit E/F record has no finite-dielectric solvent transfer or compatible CDS/nonpolar scalar, and the current registry is disabled |
| 9: legacy archive/cleanup | in progress | continuum public exports are now lazy, so dependency-light harmonic submodules do not execute legacy adapters; legacy production paths still use legacy engine assets and are not archived |
...
accuracy, complete solvation, named-solvent transfer, analytic force,
Hessian/FREQ, MD, and strict Tier V remain closed; see
