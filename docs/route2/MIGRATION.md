...
| 5: MACE-POLAR adapter/canaries | candidates complete; original common-scalar, current molecular-realspace SO(3), native-injection complete-enthalpy, and analytic changed-inference Tier-V stability routes closed negative | clean official-checkpoint canaries rule out retaining the original energy/source common scalar, isolate the pinned fixed-axis real-space operator as non-`SO(3)`, and prove that direct native-field injection omits the upstream explicit field work; the analytic replacement passes one common-state replay/envelope/rotation canary, but its reduced susceptibility is indefinite and singular, so every Tier-V profile remains unadmitted |
| 6: force/MAPLE integration | two explicit experimental daily surfaces are wired; production admission remains open | the narrow smooth-harmonic profile provides admitted experimental electrostatic E/F plus SP and first-order OPT. A separate `macemdppolarhybridddx` model now exposes the already implemented named-solvent ddX/SMD scalar through normal MAPLE E/F, molecular-virial, SP, and first-order-OPT paths. It inherits neither the harmonic admission nor the coordinate-incomplete canonical-ADT candidate |
| 7: Hessian/FREQ/TS | guarded experimental FREQ and NEB/CINEB paths implemented; production admission pending | `macemdppolarhybridddx` uses error-estimated Richardson H with bounded topology-preserving step reduction and explicitly partial PySCF-SMD topology observation. FREQ is mass-weighted vibration-only, with correct ASE/legacy units and rank-aware rigid-mode projection; no gas thermochemistry is added. E/F-only NEB/CINEB checks same-PES identity and reports convergence and candidate status separately. PRFO/NEBTS, Dimer, IRC and production FREQ/TS remain closed |
| 8: CDS/multisolvent/performance | named-solvent daily execution open; quantitative admission blocked | the ddX/SMD calculator accepts the existing Route-2 solvent registry and prevents implicit-solvent double counting. The frozen 505 development result is `1.696313 kcal/mol` MAE with a `14.904481 kcal/mol` maximum error, so production accuracy and sealed-confirmation release remain closed |
...
decomposition or force admission.

The preregistered follow-up at clean head `9c1d4fb9` evaluates total charge,
dipole, traceless quadrupole, and three deterministic far-field QM-MEP shells
from the frozen checkpoint density. All four cases pass the thresholds frozen
before execution; two processes reproduce measurement SHA-256
...
A later, separately named **operational** profile combines those unchanged
MACE-MDP permanent point multipoles with only the MACE-POLAR induced increment
and the fixed-dimensional smooth harmonic-Galerkin continuum. It makes no
quantitative-source or Tier-V claim. After the earlier GEPOL active-point
finite-difference failure was reproduced, two clean executions at candidate
head `4cf8db40` passed the frozen same-profile `h/h2/h4`, independent
directional, translation, rotation-force, and guarded closed-loop gates with
identical measurement SHA-256
`a28be11068866e035733c79064a7210739b31e17e3fbc979a5adaf22514a1a30`.
This admits only its experimental electrostatic E/F result surface. Chemical
accuracy, complete solvation, named-solvent transfer, Hessian/FREQ, MD, and
strict Tier V remain closed; see
...
Trans-butane extends that coverage to `[0,8)`; evidence is retained under
...
Dimethyl ether extends that coverage to `[0,9)`; evidence is retained under
...
Formic acid extends that coverage to `[0,10)`; evidence is retained under
...
Acetic acid extends that coverage to `[0,11)`; evidence is retained under
...
Acetaldehyde extends that coverage to `[0,12)`; evidence is retained under
...
Acetamide extends that coverage to `[0,13)`; evidence is retained under
...
Ethylamine extends that coverage to `[0,14)`; evidence is retained under
...
Pyridine extends that coverage to `[0,15)`; evidence is retained under
...
Thiophene extends that coverage to `[0,18)`; evidence is retained under
...
Methanethiol extends that coverage to `[0,19)`; evidence is retained under
