...
| 4: fixed-topology C-PCM | backend complete; unadmitted | legacy-parity adapter plus independent Torch continuum-algebra oracle; ordered-pair-frame equivariant candidate adds a complete member/frame/weight coordinate VJP; independent surface-primitive oracle missing |
| 5: MACE-POLAR adapter/canaries | candidates complete; admission gate open | the original 194-node and fixed-box40/CPCM590 paths fail rotation gates; CPCM1202 also fails methanol; a preliminary dirty-tree pair-frame/CPCM110 methanol run meets local thresholds but is not release evidence; the candidate remains unadmitted pending clean source-bound panels |
