...
| 5: solute-model adapters/canaries | MACE-POLAR candidates complete with the original common-scalar, current molecular-realspace SO(3), and analytic changed-inference Tier-V stability routes closed negative; AIMNet2 frozen-charge water candidate implemented and unadmitted | the MACE evidence boundaries are unchanged; AIMNet2 geometry-dependent NQE monopoles and coordinate JVP/VJP are composed with one finite-dielectric harmonic ddPCM scalar without a reaction-field input or electronic SCF. The transpose/KKT energy cotangent, unchanged checkpoint weights, ordinary/decomposed DFT-D3 parity, exact water configuration identity, and retained real-stack artifacts are source-bound. Every public task gate remains closed |
| 6: force/MAPLE integration | diagnostic coverage advanced; admission blocked by topology | the exact water-bound 17-molecule panel has been captured twice. All molecules pass the frozen convergence window and 12/17 pass every gate; four point/source-shell and one sphere-tangency failure keep the profile negative. A water bidirectional loop passes twice. A ten-record MNSol fixed-charge plus SMD-CDS pilot supplies early aggregate accuracy context but is not the same `G_np=0` force scalar. Public calculator/workflow and compatible nonpolar gates remain open |
| 7: Hessian/FREQ/TS | finite-ddPCM local stationary-water implementation evidence complete; admission pending | the complete weak-scalar HVP is assembled from the sealed continuum joint Hessian and AIMNet2 intrinsic/charge response. Under the exact water-bound finite-dielectric identity, a source-bound water HVP canary passes local FD/symmetry/translations twice, and a bound-transformed guarded stationary-water canary passes nine-column gradient FD, Hessian symmetry, three stationary rotations, and a correctly mass-weighted three-mode subspace twice. No vNext Tier H profile, broad event-free `C2`, multi-stationary-point FREQ, TS, or IRC admission evidence exists |
...
The legacy negative-control arm retains
`AIMNet2Calculator.charge_state/charge_position_response`. Its serialized
graph hard-casts coordinates to float32. The optional precision arm rebuilds
the official `aimnet==0.2.0` wB97M-D3 Python architecture, content-addresses
the upstream sources/configuration, and loads the same unchanged checkpoint
state dictionary in a float64 graph; it is CPU-only and research-only. The
pyddx arm retains `PyDDXPCMReactionFieldLinearMap`; the harmonic arm retains
the existing weighted `E`, Coulomb `K`, and sealed scalar/autograd kernel and
...
A guarded stationary-water search and dense-Hessian reducer now add one local
minimum, all nine HVP columns, a four-step total-gradient FD scan, six rigid
modes, and a three-mode mass-weighted vibrational subspace. Two clean
executions reproduce measurement SHA
