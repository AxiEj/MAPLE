...
| 5: MACE-POLAR adapter/canaries | candidates complete; admission gate open | the original 194-node profile retains its rotation/torque failure; the distinct fixed-box40/590-node diagnostic passes one-water Cartesian/orientation, distorted-water path/loop, equilibrium box-tail, the preregistered 20-molecule directional panel, all 465 reference-geometry Cartesian components, and the 20-reference residual-refinement gate; all-panel symmetry/loop, box-domain, and physical-component admission remain open |
| 6: force/MAPLE integration | blocked | same-scalar force is callable internally and the preregistered directional, component-resolved, and residual-refinement panels pass, but all-panel symmetry/loop, public calculator/workflow, and admission gates remain open |
