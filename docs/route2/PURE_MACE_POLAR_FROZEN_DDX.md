...
ddX active cavity changes even if the exposed-node count is unchanged.
...
- **HVP/H** are fourth-order Richardson derivatives of the same conservative
  ...
  hashes, topology identity, raw Hessian, symmetric Hessian, and raw
  antisymmetry.  Symmetrization is therefore visible rather than hidden.
