...
def _maple_source_provenance() -> dict[str, Any]:
    relative_paths = (
        Path("maple/function/dispatcher/solvfe/bulk_water.py"),
        Path("maple/function/dispatcher/solvfe/protocol.py"),
        Path("maple/function/calculator/mace/_mace_upstream_calculator.py"),
        Path("examples/solvation/route_a/validate_bulk_water.py"),
    )
    files = {}
    for relative_path in relative_paths:
        payload = (PROJECT_ROOT / relative_path).read_bytes()
        files[relative_path.as_posix()] = hashlib.sha256(payload).hexdigest()
