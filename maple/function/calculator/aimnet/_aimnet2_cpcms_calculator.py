...
def _read_json_model_card(path: Path) -> dict:
    ...
    try:
        ...
        raise ValueError(f"Model-card file at {path} must be JSON-compatible (JSON only).") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"Model-card payload at {path} must be a JSON object.")
    return payload


def _normalize_forbidden_tasks(card: dict) -> set[str]:
    raw = card.get('forbidden_tasks', ())
    if raw is None:
        return set()
    if not isinstance(raw, (list, tuple, set)):
        raise ValueError("Model-card field 'forbidden_tasks' must be a sequence.")
...
def _task_is_forbidden(task: str, *, forbidden: set[str]) -> bool:
    task = str(task).strip().lower()
    if task in {'opt', 'frequency', 'md'}:
        return any(
            label in forbidden
            for label in {task, task.replace(' ', '_'), task.replace('-', '_')}
        )
    return task in forbidden or task.replace('-', '_') in forbidden
...
def _coerce_capability_map(payload: dict) -> dict:
    ...
    if not isinstance(raw_caps, dict):
        raise ValueError("Model-card field 'capabilities' must be a mapping if present.")
    ...
    return {
        ...
        'supports_energy_derived_forces': _coerce_bool(
            raw_caps.get('supports_energy_derived_forces', top_level.get('supports_energy_derived_forces', False)),
...
def _validate_checksum(text: str, field_name: str) -> str:
    normalized = str(text).strip().lower()
    if not normalized:
        raise ValueError(f"Model-card field {field_name!r} must be a non-empty SHA256 digest.")
    if normalized == 'unknown':
        return normalized
    if len(normalized) != 64 or any(char not in '0123456789abcdef' for char in normalized):
        raise ValueError(
            f"Model-card field {field_name!r} must be a 64-hex-character SHA256 digest."
        )
    return normalized


def _validate_checkpoint_checksum(payload: dict) -> str:
    raw = payload.get(
        'checkpoint_sha256',
        payload.get('model_checksum_sha256', DEFAULT_CHECKPOINT_SHA256),
    )
    if str(raw).strip().lower() == 'unknown':
        raw = DEFAULT_CHECKPOINT_SHA256
    return _validate_checksum(raw, 'checkpoint_sha256')
...
class AIMNet2CPCMSCalculator(CalcABC):
    ...
    def __init__(
        ...
        if model_card_path is not None and Path(model_card_path).expanduser().resolve() != DEFAULT_CARD_PATH.resolve():
            raise ValueError(
        ...
        expected_checksum = _validate_checkpoint_checksum(payload)
        ...
        forbidden_tasks = _normalize_forbidden_tasks(payload)

        caps = _coerce_capability_map(payload)
        self._forbidden_tasks = forbidden_tasks
        ...
        self.model_metadata = {
            ...
            'capabilities': caps,
            'forbidden_tasks': sorted(self._forbidden_tasks),
        }

    def _validate_capability_tasks(self, properties: list) -> None:
        if 'hessian' in properties:
            if _task_is_forbidden('frequency', forbidden=self._forbidden_tasks):
                raise ValueError('AIMNet2-CPCMS model card forbids frequency workflows.')
            if not (
        ...
        if 'forces' in properties:
            if _task_is_forbidden('opt', forbidden=self._forbidden_tasks):
                raise ValueError('AIMNet2-CPCMS model card forbids optimization/forces workflows.')
            if not (
        ...
        if ('energy' in properties or 'free_energy' in properties) and _task_is_forbidden(
            'sp',
            forbidden=self._forbidden_tasks,
        ):
            raise ValueError('AIMNet2-CPCMS model card forbids single-point workflows.')

        if ('energy' in properties or 'free_energy' in properties) and not self._supports_single_point:
            raise RuntimeError(
    ...
    def _validate_request(self, atoms) -> None:
        ...
        if not math.isfinite(charge) or not charge.is_integer():
            raise ValueError(
                "AIMNet2-CPCMS requires a finite integer total molecular charge."
            )
    ...
    def _forward_energy(self, data: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if isinstance(raw, torch.Tensor):
            ...
            if 'energy' not in raw:
                raise KeyError('TorchScript LoQI model output must contain key "energy".')
            energy = raw['energy']
        else:
            raise TypeError(
    ...
    def calculate(self, atoms=None, properties=['energy'], system_changes=all_changes):
        ...
        self.results['provenance'] = {
            ...
            'scientific_status': self.model_card.get('scientific_status', 'experimental'),
        }
