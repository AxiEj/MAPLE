...
class EulerPC:
    ...
    def __init__(self, atoms: Atoms, output: str,
        ...
        if isinstance(paras, dict):
            apply_irc_parameter_overrides(
                self.p,
                select_irc_parameter_overrides(paras, "eulerpc"),
            )
    ...
    def run(self) -> Dict[str, any]:
        ...
        """
        validate_irc_params(self.p, "eulerpc")
        ...
        if self.p.write_traj:
            ...
            log_info(
                [
                    f"\n[INFO] EulerPC-IRC forward trajectory written to: {paths['forward']}\n",
                    f"[INFO] EulerPC-IRC backward trajectory written to: {paths['backward']}\n",
                    f"[INFO] EulerPC-IRC full trajectory written to: {paths['full']}\n",
                ],
                self.output,
            )
        ...
        for it in macro_steps:
            ...
            if self._early_converged:
                ...
                if irc_force_criteria_satisfied(
                    ...
                    termination_reason = "predictor_stalled"
                    log_info(
                        [
                            f"[WARNING] {title}: predictor stalled before both "
                            "endpoint force thresholds were met.\n"
                        ],
                        self.output,
                    )
                break
