...
from .harmonic_coefficients import real_wigner_matrix
from .harmonic_schwarz_primitives import (
...
def _canonical_double_layer_cross_block(
    ...
    *,
    ...
    labels = tuple(
        (ell, order)
        for ell in range(source_lmax + 1)
        for order in range(-ell, ell + 1)
    )
    cosine = _constant(reference, nodes)
    polar_weights = _constant(reference, weights)
    ...
    safe_distance = torch.where(active, radial_distance, torch.ones_like(radial_distance))
    ...
    target_design = _torch_real_harmonic_design(directions, lmax=target_lmax)
    source_design = _torch_real_harmonic_design(
        safe_directions, lmax=source_lmax
    )
    scales = []
    for ell, _ in labels:
        if ell == 0:
        ...
        scales.append(
            pair_exposure * (4.0 * np.pi / (2 * ell + 1)) * radial
        )
    ...
    stabilizer_order = azimuthal_count
    ...
    for index in range(stabilizer_order):
        ...
        target_representation = _constant(
            reference, real_wigner_matrix(rotation, lmax=target_lmax)
        )
        source_representation = _constant(
            reference, real_wigner_matrix(rotation, lmax=source_lmax)
        )
        projected = (
            projected
    ...
    for target, target_radius in enumerate(radii):
        ...
        for source, source_radius in enumerate(radii):
            ...
            if state == "buried":
                ...
                active.append(coefficients)
        rows.append(factors)
        full.append(
            _constant_coefficients(positions, lmax=partition_lmax, value=0.0)
            if buried
            else _project_product(
                tuple(active), lmax=partition_lmax, reference=positions
            )
        )
    return tuple(rows), tuple(full)


def _product_excluding_pair(
    positions: Any,
    *,
    factor_row: dict[int, tuple[str, Any]],
    excluded_source: int,
    partition_lmax: int,
):
    active = []
    for source, (state, coefficients) in factor_row.items():
        if source == excluded_source or state == "exposed":
            continue
        if state == "buried":
            return _constant_coefficients(
                positions, lmax=partition_lmax, value=0.0
            )
        active.append(coefficients)
...
def _assemble_localized_double_layer(
    ...
    *,
    ...
    factor_rows, full_exposure = _centered_exposure_factors(
        positions,
        radii=radii,
        transition_width=transition_width,
    ...
    for target, target_radius in enumerate(radii):
        for source, source_radius in enumerate(radii):
            ...
            if pair_state == "buried":
                row_blocks[target][source] = positions.new_zeros(
                    (dimension, dimension)
                )
                continue
            ...
            target_representation = _torch_wigner_matrix(
                rotation, lmax=physical_lmax
            )
            source_representation = _torch_wigner_matrix(rotation, lmax=lmax)
            pair_localized = (
                target_representation @ canonical @ source_representation.T
            )
            remaining_exposure = _product_excluding_pair(
                positions,
                factor_row=factor_rows[target],
            ...
            row_blocks[target][source] = (
                remaining_weighted_basis.T @ pair_localized
            )
