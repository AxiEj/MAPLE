...
from .harmonic_coefficients import _real_harmonic_design
...
def _assemble_smooth_partition(
    ...
    *,
    ...
    for factors in factor_rows:
        exposed = _project_product(
            tuple(one - factor for _, factor in factors),
            lmax=partition_lmax,
            reference=positions,
        )
        ...
        if count:
            ...
            nodes = 0.5 * (nodes + 1.0)
            weights = 0.5 * weights
            for atom_j, factor_j in factors:
                coefficients = positions.new_zeros(one.shape)
                for node, weight in zip(nodes, weights, strict=True):
                    product_factors = [factor_j]
                    product_factors.extend(
                        one - float(node) * factor_k
                        for atom_k, factor_k in factors
                        if atom_k != atom_j
                    )
                    coefficients = coefficients + float(weight) * _project_product(
                        tuple(product_factors),
                        lmax=partition_lmax,
                        reference=positions,
                    )
                overlap[atom_j] = coefficients

        closure = exposed
        for coefficients in overlap.values():
            closure = closure + coefficients
        closure_error = float(
            __import__("torch").max(__import__("torch").abs(closure - one)).detach().cpu()
        )
...
def _local_potential_translation(
    ...
    *,
    ...
    directions, weights = _finite_band_rule(2 * lmax)
    target_design = _constant(
        positions, _real_harmonic_design(directions, lmax=lmax)
    )
    relative = (
        positions[target]
        - positions[source]
...
def _assemble_point_ddcosmo(
    ...
    *,
    ...
    for atom_i, (exposed, _overlap) in enumerate(partition):
        ...
        source_operator[
            atom_i * dimension : (atom_i + 1) * dimension
