...
EVIDENCE = (
    ...
    / "route2"
    / "evidence"
    / "aimnet2-geometry-mediated-harmonic-ddpcm-water-4dca73d7"
)
PRIMARY = EVIDENCE / "measurements.json"
REPLAY = EVIDENCE / "cold-replay.json"

EXECUTION_HEAD = "4dca73d795f92cab3ee9d07b7c31e4b247d4e758"
EXECUTION_TREE = "f2ff7f79ccc80b1b1ce7db1efddb1b1c11b0155b"
CHECKPOINT_SHA256 = "85ba59d8c78eb4d3185f6b1614df79706427f7ca72f53f2d90e365a1723d953d"
MEASUREMENT_SHA256 = "7ec7732e79aacfd1d9502d7c19de8fd49b0b1975b764ef1cf59a413eaaf5cbbf"
NO_CAPABILITIES = {"E": False, "F": False, "H": False, "M": False, "V": False}
FILE_SHA256 = {
    "measurements.json": "b720034585f6e5fa2a59033de96c85f012b265cc30f09268409e1e9ee46a6946",
    "cold-replay.json": "f3f829d02ddb33766e06313e0eb2461953616fde617edbb458bd43d355050eb1",
    "README.md": "e32b267bdf17c9c73d1fc57ae0215e81e302c4c1ed2426d82e78e6afd53cd19c",
}
MEASURED_KEYS = (
    "protocol",
    "geometry",
...
def _recompute(artifact: dict[str, object]) -> dict[str, object]:
    ...
    common = {
        "analytic_gradient_eV_per_A": center["gradient_components_eV_per_A"][
            "total"
        ],
    ...
    decision = geometry_mediated_admission_decision(
        deterministic_replay_passed=artifact["deterministic_replay"][
            "gate_passed"
        ],
...
def _assert_artifact(artifact: dict[str, object]) -> dict[str, object]:
    ...
    assert artifact["artifact_kind"] == (
        ...
        "harmonic-ddpcm-real-stack-canary"
    )
    ...
    assert artifact["continuum_kind"] == "harmonic-ddpcm"
    ...
    assert continuum["model"] == (
        "smooth-weighted-harmonic-finite-dielectric-ddpcm"
    )
    ...
    assert max(record["energy_absolute_error_eV"] for record in rotation["records"]) == 0.0
    assert max(
...
def test_harmonic_ddpcm_artifact_is_source_bound_recomputed_and_closed():
    artifact = _load(PRIMARY)
    _assert_artifact(artifact)
    assert_source_files_match_execution_commit(ROOT, artifact)


def test_harmonic_ddpcm_cold_replay_is_scientifically_identical():
...
def test_harmonic_ddpcm_adaptive_raw_trace_rejects_energy_tampering():
    artifact = _load(PRIMARY)
    ...
    with pytest.raises(ValueError, match="SciPy result disagrees"):
        summarize_geometry_mediated_adaptive_directional_audit(
            ...
            center_continuum_topology=artifact["identity"][
...
def test_harmonic_ddpcm_evidence_file_hashes_are_frozen():
