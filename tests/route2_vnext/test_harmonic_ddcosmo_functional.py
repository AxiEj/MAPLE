...
def test_single_sphere_matches_exact_conductor_monopole_and_dipole_energy():
    ...
    expected_conductor = -0.5 * coulomb * (
        charge * charge / radius + dipole_squared / radius**3
    )
...
def test_water_converges_to_the_real_pyddx_ddcosmo_scalar():
    ...
    values = []
    for lmax in (2, 3, 4, 5):
        values.append(_candidate(atoms, WATER_RADII, lmax=lmax).energy_eV(atoms, WATER_SOURCE))
    ...
    reference = backend.prepare(atoms, WATER_SOURCE).solve(
...
def test_source_jvp_vjp_coordinate_gradient_and_hvp_share_the_scalar():
    ...
    for step in (2.0e-4, 1.0e-4, 5.0e-5):
        ...
        finite.append(
            (candidate.energy_eV(plus, WATER_SOURCE) - candidate.energy_eV(minus, WATER_SOURCE))
            / (2.0 * step)
        )
