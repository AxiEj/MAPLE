...
def test_single_sphere_matches_the_finite_dielectric_monopole_dipole_limit():
    ...
    candidate = _candidate(
        atoms, (radius,), lmax=1, dielectric=dielectric
    )
...
def test_centered_double_layer_is_differentiable_at_sphere_tangencies(separation):
    ...
    source = np.asarray(
        [[0.4, 0.05, -0.02, 0.03], [-0.4, -0.01, 0.04, -0.02]]
    )
    ...
    for step in (3.0e-4, 1.0e-4, 3.0e-5):
        ...
        errors.append(
            abs(
                (
                    candidate.energy_eV(plus, source)
                    - candidate.energy_eV(minus, source)
                )
                / (2.0 * step)
                - analytic
            )
...
def test_rotation_source_adjoint_coordinate_gradient_and_hessian_share_scalar():
    ...
    assert np.allclose(
        rotated_gradient, gradient @ rotation.T, rtol=0.0, atol=3.0e-12
    )
