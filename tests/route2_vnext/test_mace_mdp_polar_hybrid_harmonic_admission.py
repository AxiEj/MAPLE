...
MEASUREMENT_ENGINE = ROOT / "maple/solvation/release/harmonic_ef_measurement.py"
ADMISSION = (
...
def test_historical_harmonic_force_admission_does_not_open_current_registry() -> None:
...
def test_harmonic_force_admission_runner_is_parseable_and_binds_exact_profile() -> None:
    ...
    assert "run_harmonic_ef_measurement" in source


def test_harmonic_force_admission_science_engine_is_protocol_neutral() -> None:
    source = MEASUREMENT_ENGINE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(MEASUREMENT_ENGINE))
    functions = {node.name for node in tree.body if isinstance(node, ast.FunctionDef)}
    assert "run_harmonic_ef_measurement" in functions
    assert "_component_convergence_record" in functions
    assert "_closed_loop_record" in functions
    assert "_proper_rotation" in functions
    for forbidden in (
        "measurement_sha256",
        "SCHEMA_VERSION",
...
def test_replicated_admission_binds_two_clean_identical_measurements() -> None:
    ...
    assert _canonical_sha256(science) == (
