...
EVIDENCE = (
    ...
    / "route2"
    / "evidence"
    / "operational-analytic-harmonic-rigid-acetamide-ab1c7799"
)
PRIMARY = EVIDENCE / "measurements.json"
REPLAY = EVIDENCE / "cold-replay.json"
HEAD = "ab1c779946a12240eda1b54a81ef88aa05a9827d"
CHECKPOINT = "fab8b8713c832f31a2a853aaa22fd638be8a369cbf5095e6b3e982a18d10e93a"
MEASUREMENT = "3327e6e070d3ab83dd0e68c8461d6f09088d2f2fa7c423906dd03a4cd8941de7"
...
FILES = {
    ...
    "README.md": "ead5f359b37442bea664ccfbd8598eaae38bd77bcec6c69b523a055bf06c9cec",
}
...
def _assert_artifact(artifact: dict[str, object]) -> None:
    ...
    assert contract["shard_start"] == 12
    assert contract["shard_stop"] == 13
    assert contract["shard_molecule_ids"] == ["acetamide"]
    ...
    assert molecule["molecule_id"] == "acetamide"
    assert molecule["chemical_formula"] == "C2H5NO"
    ...
    assert molecule["base"]["primal_residual"] == pytest.approx(
        4.917926943572601e-13, rel=0.0, abs=1.0e-26
    )
    assert molecule["base"]["adjoint_residual"] == pytest.approx(
        3.889382009680901e-14, rel=0.0, abs=1.0e-27
    )
...
def test_acetamide_rigid_artifacts_are_bound_and_replay_exactly():
...
def test_acetamide_rigid_evidence_hashes_are_frozen():
