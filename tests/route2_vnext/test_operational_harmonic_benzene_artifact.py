...
EVIDENCE = (
    ...
    / "route2"
    / "evidence"
    / "operational-analytic-harmonic-rigid-benzene-97552baa"
)
PRIMARY = EVIDENCE / "measurements.json"
REPLAY = EVIDENCE / "cold-replay.json"
HEAD = "97552baab245acb35c6ae5a051a7c3386ea72b41"
CHECKPOINT = "fab8b8713c832f31a2a853aaa22fd638be8a369cbf5095e6b3e982a18d10e93a"
MEASUREMENT = "8f16ef863f5299eb3c7d4ace639c42d804058749740fe6713ced7477cff2f527"
...
FILES = {
    ...
    "README.md": "cc6edc30959ae047b7924760642dc054d8803c1fd4e7f5504d048a3bdf614bd6",
}
...
def _assert_artifact(artifact: dict[str, object]) -> None:
    ...
    assert contract["shard_start"] == 5
    assert contract["shard_stop"] == 6
    assert contract["shard_molecule_ids"] == ["benzene"]
    ...
    assert molecule["molecule_id"] == "benzene"
    ...
    assert molecule["base"]["primal_residual"] == pytest.approx(
        7.908002301122923e-13, rel=0.0, abs=1.0e-26
    )
    assert molecule["base"]["adjoint_residual"] == pytest.approx(
        8.328419637025845e-13, rel=0.0, abs=1.0e-26
    )
...
def test_benzene_rigid_artifacts_are_bound_and_replay_exactly():
...
def test_benzene_rigid_evidence_hashes_are_frozen():
