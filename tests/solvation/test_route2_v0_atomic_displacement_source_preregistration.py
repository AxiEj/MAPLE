...
RUNNER = BENCHMARKS / "run_route2_v0_atomic_displacement_source_acetone.py"
SOURCE = (
...
def test_atomic_displacement_source_preregistration_freezes_all_physical_inputs():
    ...
    assert protocol["execution_contract"]["source_sha256"] == {
        ...
        "run_route2_v0_atomic_displacement_source_acetone.py": _sha256(RUNNER),
    ...
    assert (
        protocol["scientific_falsification_gates"][
...
def test_atomic_displacement_source_preregistration_has_no_empirical_repair_route():
    protocol = json.loads(PREREG.read_text(encoding="utf-8"))

    assert all(value is False for value in protocol["hard_constraints"].values())
    assert "Do not change the radial data" in protocol["decision_rule"]
    assert "run an accuracy panel" in protocol["decision_rule"]
    assert protocol["source_representation"]["radial_asset"] == (
        "route2-v0-atomic-displacement-hf-def2-tzvpd-v2 with two "
        "byte-identical independent executions"
    )


def test_frozen_exterior_qm_mep_points_are_complete_and_raw_qm_bound():
