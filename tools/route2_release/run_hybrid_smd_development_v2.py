...
POINTER = Path('/tmp/maple-route2-v5-sourcebound-root.txt')
REPO_ROOT = Path(
    os.environ.get(
        'MAPLE_ROUTE2_SOURCE_ROOT',
        POINTER.read_text().strip() if POINTER.is_file() else '',
    )
).resolve()
OLD_ROOT = Path('/tmp/maple-route2-mnsol-f3.yIrFn3/repo')
BENCHMARK_ROOT = OLD_ROOT / 'docs/implicit-solvation/benchmarks'
PRIVATE_PANEL = Path('/tmp/maple-route2-mnsol-f3.yIrFn3/fixed-arm-validation-v1-run3/private.json')
DATASET_SOURCE = OLD_ROOT / '.omx/datasets/mnsol-v2012/MNSolDatabase_v2012.zip'
PROTOCOL_PATH = BENCHMARK_ROOT / 'route2-mnsol-protocol-v1.json'
SELECTION_PATH = OLD_ROOT / '.omx/benchmarks/route2-mnsol-development-selection-v1.private.json'
PILOT_PATH = BENCHMARK_ROOT / 'route2-mnsol-pilot-selection-v1.json'
...
if not REPO_ROOT.is_dir():
    raise RuntimeError('MAPLE_ROUTE2_SOURCE_ROOT does not identify the isolated source clone.')
...
def _validate_preregistration(prereg: Mapping[str, Any]) -> None:
    if prereg.get('artifact_id') != 'route2-hybrid-smd-development-prereg-v2':
        raise ValueError('Unknown preregistration identity.')
    if prereg.get('status') != 'locked-before-first-v2-hybrid-evaluation':
    ...
    expected_hashes = {
        'dataset_zip_sha256': _sha256(DATASET_SOURCE),
        'protocol_sha256': _sha256(PROTOCOL_PATH),
        'development_selection_sha256': _sha256(SELECTION_PATH),
        'pilot_selection_sha256': _sha256(PILOT_PATH),
        'private_development_panel_sha256': _sha256(PRIVATE_PANEL),
...
def _validate_panel(panel: Mapping[str, Any]) -> dict[int, Mapping[str, Any]]:
    if panel.get('do_not_commit') is not True or panel.get('partition') != 'development':
        raise ValueError('Private development panel contract is invalid.')
    selection = panel.get('selection')
    if not isinstance(selection, Mapping):
        raise ValueError('Private selection metadata is missing.')
    if selection.get('confirmation_partition_opened') is not False:
        raise ValueError('Sealed confirmation partition was opened.')
    if selection.get('record_count') != EXPECTED_RECORD_COUNT:
        raise ValueError('Private development record count drifted.')
    records = panel.get('records')
    if not isinstance(records, list) or len(records) != EXPECTED_RECORD_COUNT:
        raise ValueError('Private development records are incomplete.')
    indexed = {int(record['selection_index']): record for record in records}
    if tuple(sorted(indexed)) != tuple(range(EXPECTED_RECORD_COUNT)):
        raise ValueError('Private development indices are not exactly 0..504.')
    return indexed
...
def main() -> int:
    ...
    if args.mdp_checkpoint.resolve() != MDP_CHECKPOINT.resolve():
        raise ValueError('MACE-MDP checkpoint path differs from the preregistered file.')
    if args.polar_checkpoint.resolve() != POLAR_CHECKPOINT.resolve():
        raise ValueError('MACE-POLAR checkpoint path differs from the preregistered file.')
    prereg = json.loads(PREREG_PATH.read_text())
    _validate_preregistration(prereg)
    prereg_sha256 = _sha256(PREREG_PATH)
    runner_sha256 = _sha256(Path(__file__))
    mdp_checkpoint_sha256 = _sha256(args.mdp_checkpoint)
    polar_checkpoint_sha256 = _sha256(args.polar_checkpoint)

    panel = json.loads(PRIVATE_PANEL.read_text())
    expected_by_index = _validate_panel(panel)
    protocol = load_mnsol_protocol(PROTOCOL_PATH)
    dataset = load_mnsol_v2012(DATASET_SOURCE, protocol)
    ...
    for ordinal, selection_index in enumerate(range(args.start, args.stop), start=1):
        output = args.output_dir / f'index-{selection_index:03d}.json'
        ...
        expected = expected_by_index[selection_index]
        geometry = selected.eligible_record.geometry
        if (
            selected.opaque_record_id != expected['opaque_record_id']
            or geometry.sha256 != expected['geometry_sha256']
            or selected.canonical_solvent != expected['canonical_solvent']
        ):
            raise RuntimeError(f'Frozen selection identity drift at {selection_index}.')
        atoms = Atoms(
            numbers=geometry.atomic_numbers,
            positions=geometry.coordinates_angstrom,
            info={'charge': 0, 'mult': 1},
        )
        experimental = float(expected['experimental_delta_g_kcal_mol'])
        record: dict[str, Any] = {
    ...
    summary = {
        'artifact': 'route2-hybrid-smd-development-shard-v2',
    ...
    _write_json_atomic(
        args.output_dir / f'shard-summary-{args.start:03d}-{args.stop:03d}.json',
        summary,
    )
