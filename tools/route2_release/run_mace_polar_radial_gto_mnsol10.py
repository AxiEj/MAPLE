...
POINT_L1_HARMONIC_EMBEDDING = "point-l1-harmonic"
POINT_L1_HARMONIC_DDCOSMO_EMBEDDING = "point-l1-harmonic-ddcosmo"
POINT_L1_HARMONIC_DDPCM_EMBEDDING = "point-l1-harmonic-ddpcm"
SUPPORTED_SOURCE_EMBEDDINGS = (
    RADIAL_GTO_EMBEDDING,
    POINT_L1_EMBEDDING,
    POINT_L1_HARMONIC_EMBEDDING,
    POINT_L1_HARMONIC_DDCOSMO_EMBEDDING,
    POINT_L1_HARMONIC_DDPCM_EMBEDDING,
)
PROFILE_IDENTITIES = {
    RADIAL_GTO_EMBEDDING: {
    ...
    POINT_L1_HARMONIC_EMBEDDING: {
        "artifact_id": (
            "route2-mnsol10-macepolar-point-l1-frozen-"
            "smoothharmonicgalerkin-smd-v1"
        ),
        "profile_id": (
            "route2-pure-macepolar-point-l1-frozen-"
            "smoothharmonicgalerkin-smd-v1"
        ),
        "private_name": "mnsol10-point-l1-harmonic-private-v1.json",
        "public_name": "mace-polar-point-l1-harmonic-mnsol10-accuracy-v1.json",
    },
    POINT_L1_HARMONIC_DDCOSMO_EMBEDDING: {
        "artifact_id": (
            "route2-mnsol10-macepolar-point-l1-frozen-"
        ...
        "private_name": "mnsol10-point-l1-harmonic-ddcosmo-private-v1.json",
        "public_name": (
            "mace-polar-point-l1-harmonic-ddcosmo-mnsol10-accuracy-v1.json"
        ),
    },
    POINT_L1_HARMONIC_DDPCM_EMBEDDING: {
        "artifact_id": (
        ...
        "private_name": "mnsol10-point-l1-harmonic-ddpcm-private-v1.json",
        "public_name": (
            "mace-polar-point-l1-harmonic-ddpcm-mnsol10-accuracy-v1.json"
        ),
    },
}
...
HARMONIC_DDCOSMO_PREREGISTRATION = (
    BENCHMARK_DIR / "route2-mnsol10-harmonic-ddcosmo-preregistration-v1.json"
)
HARMONIC_DDPCM_PREREGISTRATION = (
    BENCHMARK_DIR / "route2-mnsol10-harmonic-ddpcm-preregistration-v1.json"
)
SOURCE_FILES = (
    "GOAL.md",
    ...
    "maple/solvation/continuum/harmonic_ddcosmo_functional.py",
...
def _validate_harmonic_ddcosmo_preregistration(args: argparse.Namespace) -> str:
    payload = json.loads(HARMONIC_DDCOSMO_PREREGISTRATION.read_text(encoding="utf-8"))
    expected = payload["profile"]
    observed = {
        "source_embedding": str(args.source_embedding),
        "surface_lmax": int(args.harmonic_surface_lmax),
        "partition_lmax": int(args.harmonic_exposure_lmax),
        "transition_width_angstrom2": float(
            args.harmonic_transition_width_angstrom2
        ),
        "partition_radial_quadrature_order": int(
            args.harmonic_exposure_radial_order
        ),
        "source_radial_quadrature_order": int(args.harmonic_source_radial_order),
    }
    for name, value in observed.items():
        if value != expected[name]:
            raise ValueError(
                "harmonic ddCOSMO run differs from its preregistration: "
                f"{name}={value!r}, expected {expected[name]!r}."
            )
    return sha256_file(HARMONIC_DDCOSMO_PREREGISTRATION)


def _validate_harmonic_ddpcm_preregistration(args: argparse.Namespace) -> str:
    payload = json.loads(HARMONIC_DDPCM_PREREGISTRATION.read_text(encoding="utf-8"))
    expected = payload["profile"]
    observed = {
        "source_embedding": str(args.source_embedding),
        "surface_lmax": int(args.harmonic_surface_lmax),
        ...
        "double_layer_radial_quadrature_order": int(
            args.harmonic_double_layer_radial_order
        ),
    }
    for name, value in observed.items():
        if value != expected[name]:
            raise ValueError(
                "harmonic ddPCM run differs from its preregistration: "
                f"{name}={value!r}, expected {expected[name]!r}."
            )
    return sha256_file(HARMONIC_DDPCM_PREREGISTRATION)
...
def _point_harmonic_state(
    ...
    *,
    ...
    """Evaluate the frozen point-source smooth-harmonic scalar for one row."""

    count = len(atoms)
    zero = np.zeros((count, 8), dtype=float)
    source_state = model.evaluate_source(
        atoms, zero, need_fixed_field_forces=False
    )
    source8 = np.asarray(getattr(source_state, "source", None), dtype=float)
    if source8.shape != (count, 8) or not np.all(np.isfinite(source8)):
        raise RuntimeError("MACE-POLAR zero-field source is invalid.")
    if np.any(source8[:, (1, 5, 6, 7)] != 0.0):
        raise RuntimeError(
            "point-harmonic profile requires an exact zero unused radial block."
        )
    source4 = np.ascontiguousarray(source8[:, (0, 2, 3, 4)], dtype=float)
    specification = route2_solvent_spec(solvent)
    symbols = tuple(atoms.get_chemical_symbols())
    radii = tuple(
        float(value) for value in smd_coulomb_radii(symbols, solvent=solvent)
    )
    continuum = SmoothWeightedHarmonicGalerkinFunctionalCandidate(
        ...
        transition_width_angstrom2=args.harmonic_transition_width_angstrom2,
        surface_lmax=args.harmonic_surface_lmax,
        exposure_lmax=args.harmonic_exposure_lmax,
        exposure_radial_quadrature_order=args.harmonic_exposure_radial_order,
        source_radial_quadrature_order=args.harmonic_source_radial_order,
        green_radial_quadrature_order=args.harmonic_green_radial_order,
        dtype=model.dtype,
        device=model.device,
    ...
    polarization = continuum.energy_eV(atoms, source4)
    cds_term = PySCFSMDCDSTerm(symbols, solvent)
    cds_state = cds_term.evaluate(atoms, need_gradient=False)
    source_charge = float(np.sum(source4[:, 0]))
    continuum_state_sha256 = canonical_metadata_sha256(
        {
            "contract": "frozen-point-l1-smooth-harmonic-energy-state-v1",
            "geometry_sha256": getattr(source_state, "geometry_sha256", None),
            "continuum_configuration_sha256": continuum.configuration_sha256(),
            "continuum_topology_sha256": continuum.topology_sha256(),
            "source_sha256": hashlib.sha256(source4.astype("<f8").tobytes()).hexdigest(),
            "polarization_energy_eV": polarization,
        }
    )
    state_sha256 = canonical_metadata_sha256(
        {
            "continuum_state_sha256": continuum_state_sha256,
            "solvent_term_state_sha256": cds_state.state_sha256,
    ...
    return {
        "configuration_sha256": canonical_metadata_sha256(
            {
                "continuum_configuration_sha256": continuum.configuration_sha256(),
                "solvent_term_configuration_sha256": cds_term.configuration_sha256(),
                "source_extraction": "radial8-columns-[0,2,3,4]-to-point-l1",
            }
        ),
        "polarization_energy_eV": polarization,
        "cds_energy_eV": cds_state.energy_eV,
        "source_total_charge_e": source_charge,
...
def _point_harmonic_ddcosmo_state(
    model: object,
    atoms: Atoms,
    *,
    solvent: str,
    ...
    """Evaluate the fixed-coefficient smooth-partition ddCOSMO scalar."""
    ...
    if np.any(source8[:, (1, 5, 6, 7)] != 0.0):
        raise RuntimeError(
            "point harmonic ddCOSMO requires an exact zero unused radial block."
        )
    source4 = np.ascontiguousarray(source8[:, (0, 2, 3, 4)], dtype=float)
    specification = route2_solvent_spec(solvent)
    symbols = tuple(atoms.get_chemical_symbols())
    ...
    continuum = SmoothPartitionHarmonicDDCOSMOFunctionalCandidate(
        atomic_numbers=tuple(int(value) for value in atoms.numbers),
        radii_angstrom=radii,
        transition_width_angstrom2=args.harmonic_transition_width_angstrom2,
        surface_lmax=args.harmonic_surface_lmax,
        partition_lmax=args.harmonic_exposure_lmax,
        partition_radial_quadrature_order=args.harmonic_exposure_radial_order,
        source_radial_quadrature_order=args.harmonic_source_radial_order,
        dielectric=specification.descriptors.dielectric,
        dtype=model.dtype,
        device=model.device,
    ...
    continuum_state_sha256 = canonical_metadata_sha256(
        {
            "contract": "frozen-point-l1-smooth-harmonic-ddcosmo-state-v1",
            "geometry_sha256": getattr(source_state, "geometry_sha256", None),
            "continuum_configuration_sha256": continuum.configuration_sha256(),
            "source_sha256": hashlib.sha256(
                source4.astype("<f8").tobytes()
            ).hexdigest(),
            "polarization_energy_eV": polarization,
        }
    )
    ...
    configuration = continuum.configuration_sha256()
    return {
        "configuration_sha256": canonical_metadata_sha256(
            {
                "continuum_configuration_sha256": configuration,
                "solvent_term_configuration_sha256": cds_term.configuration_sha256(),
                "source_extraction": "radial8-columns-[0,2,3,4]-to-point-l1",
            }
        ),
        ...
        "topology_id": canonical_metadata_sha256(
            {
                "contract": "fixed-local-potential-coefficient-topology-v1",
                "atom_count": count,
                "surface_lmax": args.harmonic_surface_lmax,
                "owner_order": "input-atom-order",
            }
        ),
    }


def _point_harmonic_ddpcm_state(
    model: object,
    atoms: Atoms,
    *,
    solvent: str,
    ...
    """Evaluate the preregistered finite-dielectric harmonic ddPCM scalar."""
    ...
    if np.any(source8[:, (1, 5, 6, 7)] != 0.0):
        raise RuntimeError(
            "point harmonic ddPCM requires an exact zero unused radial block."
        )
    source4 = np.ascontiguousarray(source8[:, (0, 2, 3, 4)], dtype=float)
    specification = route2_solvent_spec(solvent)
    symbols = tuple(atoms.get_chemical_symbols())
    ...
    continuum_state_sha256 = canonical_metadata_sha256(
        {
            "contract": "frozen-point-l1-smooth-harmonic-ddpcm-state-v1",
            "geometry_sha256": getattr(source_state, "geometry_sha256", None),
            "continuum_configuration_sha256": continuum.configuration_sha256(),
            "source_sha256": hashlib.sha256(
                source4.astype("<f8").tobytes()
    ...
    return {
        ...
        "topology_id": canonical_metadata_sha256(
            {
                "contract": "fixed-harmonic-ddpcm-coefficient-topology-v1",
                "atom_count": count,
                "surface_lmax": args.harmonic_surface_lmax,
                "owner_order": "input-atom-order",
            }
...
def run(args: argparse.Namespace) -> tuple[dict[str, object], dict[str, object]]:
    source_embedding = str(args.source_embedding)
    if source_embedding == POINT_L1_HARMONIC_DDCOSMO_EMBEDDING:
        preregistration_sha256 = _validate_harmonic_ddcosmo_preregistration(args)
    elif source_embedding == POINT_L1_HARMONIC_DDPCM_EMBEDDING:
        preregistration_sha256 = _validate_harmonic_ddpcm_preregistration(args)
    else:
    ...
    for ordinal, selected in enumerate(selection, start=1):
        ...
        if source_embedding in {
            POINT_L1_HARMONIC_EMBEDDING,
            POINT_L1_HARMONIC_DDCOSMO_EMBEDDING,
            POINT_L1_HARMONIC_DDPCM_EMBEDDING,
        }:
            evaluator = {
                POINT_L1_HARMONIC_EMBEDDING: _point_harmonic_state,
                POINT_L1_HARMONIC_DDCOSMO_EMBEDDING: (
                    _point_harmonic_ddcosmo_state
                ),
                POINT_L1_HARMONIC_DDPCM_EMBEDDING: _point_harmonic_ddpcm_state,
            }[source_embedding]
            state = evaluator(
                model,
                atoms,
            ...
            configuration_sha256 = str(state["configuration_sha256"])
            polarization_energy_eV = float(state["polarization_energy_eV"])
            cds_energy_eV = float(state["cds_energy_eV"])
            source_total_charge_e = float(state["source_total_charge_e"])
            state_sha256 = str(state["state_sha256"])
            continuum_state_sha256 = str(state["continuum_state_sha256"])
            solvent_term_state_sha256 = str(state["solvent_term_state_sha256"])
        ...
        predicted = (polarization_energy_eV + cds_energy_eV) * EV_TO_KCAL_MOL
        experimental = float(item.record.delta_g_kcal_mol)
        configuration_hashes[selected.canonical_solvent] = configuration_sha256
        rows.append(
            {
                ...
                "polarization_kcal_mol": (
                    polarization_energy_eV * EV_TO_KCAL_MOL
                ),
                "cds_kcal_mol": cds_energy_eV * EV_TO_KCAL_MOL,
                "source_total_charge_e": source_total_charge_e,
                "state_sha256": state_sha256,
                "continuum_state_sha256": continuum_state_sha256,
                "solvent_term_state_sha256": solvent_term_state_sha256,
    ...
    profile_payload = {
        ...
        "continuum_model": (
            {
                POINT_L1_HARMONIC_DDCOSMO_EMBEDDING: (
                    "smooth-partition-harmonic-ddcosmo"
                ),
                POINT_L1_HARMONIC_DDPCM_EMBEDDING: (
                    "smooth-partition-harmonic-ddpcm"
                ),
            }.get(source_embedding, "pcm")
        ),
        "lmax": args.lmax,
        "n_lebedev": args.n_lebedev,
        "solver_tolerance": args.solver_tolerance,
        "eta": args.eta,
        "n_proc": args.n_proc,
        "harmonic_surface_lmax": args.harmonic_surface_lmax,
        "harmonic_exposure_lmax": args.harmonic_exposure_lmax,
        "harmonic_transition_width_angstrom2": (
            args.harmonic_transition_width_angstrom2
        ),
        "harmonic_exposure_radial_order": args.harmonic_exposure_radial_order,
        "harmonic_source_radial_order": args.harmonic_source_radial_order,
        "harmonic_green_radial_order": args.harmonic_green_radial_order,
        "harmonic_double_layer_radial_order": (
            args.harmonic_double_layer_radial_order
        ),
        "selection_fingerprint": selection_manifest["selection_fingerprint"],
        "protocol_fingerprint": protocol.fingerprint,
        "source_files_sha256": source_hashes,
        "harmonic_preregistration_sha256": preregistration_sha256,
    }
    ...
    common = {
        ...
        "harmonic_ddcosmo_preregistration": (
            {
                "artifact": HARMONIC_DDCOSMO_PREREGISTRATION.name,
        ...
        "harmonic_ddpcm_preregistration": (
            {
                "artifact": HARMONIC_DDPCM_PREREGISTRATION.name,
                "sha256": preregistration_sha256,
            }
            if source_embedding == POINT_L1_HARMONIC_DDPCM_EMBEDDING
            else None
        ),
        ...
        "scientific_identity": {
            ...
            "source_embedding": source_embedding,
            "continuum": {
                RADIAL_GTO_EMBEDDING: "pyddx ddPCM radial-GTO general source",
                POINT_L1_EMBEDDING: "pyddx ddPCM point-l<=1 multipole source",
                POINT_L1_HARMONIC_EMBEDDING: (
                    "smooth weighted harmonic-Galerkin CPCM with differentiable "
                    "point-l<=1 source coefficients"
                ),
                POINT_L1_HARMONIC_DDCOSMO_EMBEDDING: (
                    "smooth-partition harmonic ddCOSMO with fixed local-potential "
                    "coefficients and point-l<=1 source"
                ),
                POINT_L1_HARMONIC_DDPCM_EMBEDDING: (
                    "finite-dielectric smooth-partition harmonic ddPCM with "
                    "centered coefficient-space double layer and point-l<=1 source"
                ),
            }[source_embedding],
            ...
            "energy_ledger": (
                "G_harmonic-CPCM[frozen zero-field source] + G_SMD-CDS"
                if source_embedding == POINT_L1_HARMONIC_EMBEDDING
                else (
                    "G_smooth-harmonic-ddCOSMO[frozen zero-field source] + "
                    "G_SMD-CDS"
                    if source_embedding == POINT_L1_HARMONIC_DDCOSMO_EMBEDDING
                    else (
                        "G_smooth-harmonic-ddPCM[frozen zero-field source] + "
                        "G_SMD-CDS"
                        if source_embedding == POINT_L1_HARMONIC_DDPCM_EMBEDDING
                        else "G_ddPCM[frozen zero-field source] + G_SMD-CDS"
                    )
                )
            ),
        ...
        "continuum_parameters": (
            {
                "model": (
                    ...
                    }.get(source_embedding, "smooth-harmonic-galerkin-cpcm")
                ),
                "surface_lmax": args.harmonic_surface_lmax,
                (
                    "partition_lmax"
                ...
                "source_radial_quadrature_order": (
                    args.harmonic_source_radial_order
                ),
                "green_radial_quadrature_order": args.harmonic_green_radial_order,
                "double_layer_radial_quadrature_order": (
                    args.harmonic_double_layer_radial_order
        ...
        "claim_boundary": (
            ...
            f"the exact pure MACE-POLAR {source_embedding} continuum plus SMD-CDS "
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--harmonic-surface-lmax", type=int, default=1)
    parser.add_argument("--harmonic-exposure-lmax", type=int, default=2)
    parser.add_argument(
        "--harmonic-transition-width-angstrom2", type=float, default=0.18
    )
    parser.add_argument("--harmonic-exposure-radial-order", type=int, default=32)
    parser.add_argument("--harmonic-source-radial-order", type=int, default=64)
    parser.add_argument("--harmonic-green-radial-order", type=int, default=32)
    parser.add_argument("--harmonic-double-layer-radial-order", type=int, default=128)
    return parser
