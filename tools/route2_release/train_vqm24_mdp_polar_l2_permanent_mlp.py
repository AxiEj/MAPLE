#!/usr/bin/env python3
"""Train the final allowed Q-B permanent head with two frozen POLAR l2 carriers."""
...
from tools.route2_release import (  # noqa: E402
    train_vqm24_mdp_gaussian_l2_permanent_mlp as qa,
)
from tools.route2_release import (  # noqa: E402
...
SELF_REPO_PATH = "tools/route2_release/train_vqm24_mdp_polar_l2_permanent_mlp.py"
ARTIFACT = "route2-vqm24-mdp-polar-l2-permanent-local-mlp-v1"


class PermanentPolarL2MLP(qa.PermanentLocalMLP):
    def __init__(self) -> None:
        super().__init__()
        self.atom_head[-1] = torch.nn.Linear(
            qa.HIDDEN_WIDTH,
            8,
            dtype=torch.float64,
        )
    ...
    def forward(self, data: dict[str, Any]):
            ...
            for atom_j in range(atom_i + 1, atom_count):
                ...
                cutoff = torch.where(
                    ...
                    * (
                        1.0
                        + torch.cos(
                            np.pi * distance / base.PAIR_CUTOFF_ANGSTROM
                        )
                    ),
        ...
        quadrupole = (
            ...
            + polar_gain[:, 0, None] * data["polar_l2"][:, 0]
            + polar_gain[:, 1, None] * data["polar_l2"][:, 1]
        )
        mep = (
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    polar_root = args.polar_l2_features.expanduser().resolve(strict=True)
    ...
    for record in preregistration["records"]:
        data = base._record_data(record, mdp_root=mdp_root, device=device)
        with np.load(
            polar_root / record["record_id"] / "carriers.npz",
            allow_pickle=False,
        ) as state:
            carriers = np.asarray(state["l2_carriers"], dtype=np.float64)
        if carriers.shape != (len(data["positions"]), 2, 5):
            raise RuntimeError("Frozen POLAR l2 carrier shape changed.")
        data["polar_l2"] = torch.as_tensor(
            carriers,
            dtype=torch.float64,
            device=device,
        )
    ...
    model = PermanentPolarL2MLP().to(device=device)
    parameter_count = sum(parameter.numel() for parameter in model.parameters())
    if parameter_count > 512:
        raise RuntimeError("Q-B local MLP exceeds the frozen 512-parameter cap.")
    ...
    torch.save(
        {
            ...
            "mep_scale": mep_scale,
            "dipole_scale": dipole_scale,
            "polar_l2_manifest_sha256": base._sha256(polar_root / "manifest.json"),
        },
        checkpoint_path,
    )
    payload: dict[str, Any] = {
        ...
        "source": {
            ...
            "qa_runner_sha256": base._sha256(SOURCE_ROOT / qa.SELF_REPO_PATH),
        ...
        "architecture": {
            "name": "Q-B-local-invariant-MLP-plus-two-frozen-POLAR-l2",
            "trainable_parameter_count": parameter_count,
            "hidden_width": qa.HIDDEN_WIDTH,
            "new_message_passing": False,
            "backbone_parameters_trainable": False,
            "frozen_polar_l2_carrier_count": 2,
        },
        "training": {
            "device": str(device),
            "dtype": "float64",
        ...
        "metrics": metrics,
        "aggregate": {
            "mean_fit_mep_relative": float(np.mean([m["fit_mep_relative"] for m in metrics])),
            "mean_audit_mep_relative": float(np.mean([m["audit_mep_relative"] for m in metrics])),
            "maximum_audit_mep_relative": max(m["audit_mep_relative"] for m in metrics),
            "mean_dipole_relative": float(np.mean([m["dipole_relative"] for m in metrics])),
            "maximum_charge_residual_e": max(m["charge_residual_e"] for m in metrics),
        },
        "runtime": {
...
def main() -> None:
    ...
    parser.add_argument("--polar-l2-features", type=Path, required=True)
